...
from verl.utils.reward_score import gsm8k, math_verify, table_area, MATH
...
def _select_rm_score_fn(data_source):
    ...
    elif data_source == "table_area":
        return table_area.compute_score
    else:
        raise NotImplementedError


def compute_score(data_source, solution_str, ground_truth, prompt_str, step, extra_info=None):
    print("\n" + "=" * 80)
    ...
    res, s1, s2 = compute_score_fn(solution_str, ground_truth, prompt_str, step)

    print(f"  Total Score: {res}")
    print("=" * 80 + "\n")

    if isinstance(res, (int, float, bool)):
        return float(res), float(s1), float(s2)
    else:
        return float(res[0])
